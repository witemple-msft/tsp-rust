...
export function createResultInfo(
  ...
  for (const response of responses) {
    ...
    output.push(
      ...indent(
        indent(emitErrorProcessingCode(ctx, responseContent, operationCase))
      )
    );
    output.push("  },");
  }
  ...
  output.push(
    ...indent(
      indent(emitSuccessProcessingCode(ctx, successResponse, operationCase))
    )
  );
  output.push("  },");
  ...
  output.push(
    ...indent(
      indent(emitErrorProcessingCode(ctx, errorResponse, operationCase))
    )
  );
  output.push("  },");
...
function* emitSuccessProcessingCode(
  ctx: RustContext,
  response: HttpOperationResponseContent,
  operationCase: ReCase
): Iterable<string> {
  if (response.body) {
    yield `let body = res.json::<${emitTypeReference(
      ctx,
      response.body?.type,
      response.body?.type,
      "owned",
      "models::",
      operationCase.pascalCase + "ResponseBody"
    )}>().await?;`;
    yield "";
    yield "Ok(body)";
  } else {
...
function* emitErrorProcessingCode(
  ctx: RustContext,
  response: HttpOperationResponseContent,
  operationCase: ReCase
): Iterable<string> {
