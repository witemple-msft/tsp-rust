...
import { createPathCursor } from "./ctx.js";
import { parseCase } from "./util/case.js";
import { createOrGetModuleForNamespace, visitAllTypes, } from "./common/namespace.js";
...
export async function $onEmit(context) {
    ...
    let namespacePath = [];
    let namespace = service.type;
    while (namespace !== globalNamespace) {
        namespacePath.push(namespace);
        if (!namespace.namespace) {
            throw new Error("UNREACHABLE: failed to encounter global namespace in namespace traversal");
        }
    ...
    while (!rustCtx.typeQueue.isEmpty() || rustCtx.synthetics.length > 0) {
        ...
        while (rustCtx.synthetics.length > 0) {
            ...
            syntheticModule.declarations.push([
                ...emitDeclaration(rustCtx, synthetic.underlying, syntheticModule.cursor, synthetic.name),
            ]);
        }
    }
    try {
    ...
    await writeModuleTree(rustCtx, context.emitterOutputDir, rootModule, context.options["output-mode"]);
}
