...
export function* emitOperation(ctx, operation) {
    ...
    const [requiredParameters, optionalParameters] = bifilter(parameters, function (param) {
        return !param.param.optional;
    });
    const bodyTypeReference = body &&
        emitTypeReference(ctx, body.type, body.parameter ?? NoTarget, "owned", "models::", operationNameCase.pascalCase + "RequestBody");
    const bodyIsRequired = body && !body.parameter?.optional;
    const bodyIsOptional = body && body.parameter?.optional;
    const bodyFieldName = body && parseCase(body.parameter?.name ?? "body").snakeCase;
    ...
    if (operation.responses.length > 1)
        debugger;
    const resultInfo = createResultInfo(ctx, operation.responses, operationNameCase);
    const params = requiredParameters.map(function createParamBinding(param) {
        const nameCase = parseCase(param.param.name);
        const name = nameCase.snakeCase;
        const typeReference = emitTypeReference(ctx, param.param.type, param.param, "param", "models::", operationNameCase.pascalCase + nameCase.pascalCase);
        return `${name}: ${typeReference}`;
    });
    const pathTemplate = operation.path.replace(/{([^}]+)}/g, function recase_parameter(subst) {
    ...
    const requiredQueryParameters = requiredParameters.filter((param) => param.type === "query");
    const queryExpressions = [];
    const requiredQueryParts = requiredQueryParameters.map(function queryParam(param) {
    ...
    const paramLine = queryExpressions.join("") +
        (params.length > 0 ? ", " + params.join(", ") : "") +
        bodyParam;
    const queryFormatString = requiredQueryParts.length > 0 ? `?${requiredQueryParts.join("&")}` : "";
    const requiredHeaderPreparation = [];
    for (const headerParam of requiredParameters.filter((param) => param.type === "header")) {
        const name = parseCase(headerParam.param.name).snakeCase;
        requiredHeaderPreparation.push(`.header(${vendoredModulePath("reqwest", "header", "HeaderName")}::from_static("${headerParam.name}"), ${name})`);
    }
    const requestPreparation = [
        `let res = ctx.client.${method.toLowerCase()}(&path)`,
    ];
    requestPreparation.push(...indent(requiredHeaderPreparation), ...indent(requiredBodyLines), "  .send()", "  .await?;");
    // prettier-ignore
    ...
    if (optionalParameters.length > 0 || bodyIsOptional) {
        ...
        const optionalQueryParameters = optionalParameters.filter((param) => param.type === "query");
        const allQueryParts = [...requiredQueryParts];
        if (optionalQueryParameters.length > 0) {
            allQueryParts.push("{}");
        }
        const queryFormatBinding = optionalQueryParameters.length > 0 ? ", options.query_string()" : "";
        const queryFormatString = allQueryParts.length > 0 ? `?${allQueryParts.join("&")}` : "";
