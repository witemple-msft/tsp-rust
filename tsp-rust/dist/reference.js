...
export function emitTypeReference(ctx, type, position, disposition, cursor, preferredAlternativeName) {
    switch (type.kind) {
        case "Scalar":
            return getRustScalar(ctx.program, type, position)[disposition];
        case "Model": {
            if (isWellKnownModel(ctx, type)) {
                return emitWellKnownModel(ctx, type, disposition, cursor, preferredAlternativeName);
            }
            ...
            if (effectiveModel.name === "") {
                ...
                const name = cursor.resolveAbsolutePath("models", "synthetic", preferredAlternativeName);
            ...
            const templatedName = parseCase(effectiveModel.templateMapper
                ? effectiveModel.name +
                    effectiveModel
